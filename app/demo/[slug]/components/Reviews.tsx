...
function Stars({ size = "w-4 h-4" }: { size?: string }) {
  return (
    <div className="flex gap-0.5 mb-3">
      {[...Array(5)].map((_, i) => (
        <svg key={i} className={`${size} text-[#c9432c]`} fill="currentColor" viewBox="0 0 20 20">
    ...
    <section className="relative px-6 py-16" style={{ background: "rgba(255,255,255,0.015)" }}>
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-10">
          <p className="text-xs font-bold uppercase tracking-widest text-[#b0271a] mb-2">Customer reviews</p>
          <h2 className="display-font text-2xl sm:text-3xl font-bold text-white mb-4">Trusted by Homeowners in Millbrook and Beyond</h2>
          <div className="flex flex-col items-center gap-1.5">
            <Stars size="w-5 h-5" />
            <p className="text-white/50 text-sm">
              <span className="text-white font-bold">4.9 out of 5</span> — from 300+ deliveries this season
            </p>
          </div>
        </div>
